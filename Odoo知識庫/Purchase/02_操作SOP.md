...
> 更新：2026-08-05 於實際 v19-ce 沙盒環境驗證更正 PAL03 Run Scheduler 路徑（影片路徑v19已失效），並新增 Step 4 Replenishment 報表操作說明。
...
> ⚠️ **路徑更正（2026-08-05 實測 v19-ce 確認）**：以下 [04:23]／[06:12] 為 Odoo 17 影片路徑，**v19 已移除「Inventory → Operations → Run Scheduler」這個選單位置**，實際找不到。v19 正確路徑如下，請以此為準。

**v19 正確路徑（手動觸發全部產品）— 官方正規路徑（2026-08-05對照官方文件＋Jenny實際畫面雙重確認，優先用這條）**：
1. 開啟 Developer Mode（開發者模式）
2. Inventory（庫存）App → 製程（Operations）→ 補貨（Replenishment）子選單 → 「補貨：執行調度器（Procurement: Run Scheduler）」
3. 點「手動執行」
4. 這一次會把**所有**符合條件（低於Min）的產品一次觸發，不是單一產品

**替代路徑（同一個動作，殊途同歸）**：Settings（設定）→ Technical（技術）→ 自動化 → 安排的動作（Scheduled Actions）→ 搜尋「補貨」找到同一筆記錄 → 手動執行（⚠️「排程動作」為之前猜測翻譯之誤，已對照Jenny實際畫面更正為「安排的動作」）

> 📌 這個排程動作預設「定時執行（Execute Every）」= 1天，代表 Odoo 背景每天自動跑一次；上述手動執行是額外立即觸發一次，不影響原本的每日排程。示範/測試用途建議用手動執行，不建議調整定時執行的頻率（改了要記得改回來，且會影響全公司走此排程的所有品項，不是只影響你在測試的品項）。

（原 [04:23] 單一產品觸發路徑「產品 → Reordering Rules → 點規則本身 → Run Scheduler」待確認 v19 是否仍存在對應按鈕；v19 建議改用 Step 4 補貨報表上的「訂單」圖示對單一品項手動觸發，效果等同且路徑已確認可用）

### Step 4｜Replenishment（補貨）報表操作

路徑：Inventory（庫存）App → Reporting（報告）→ Replenishment（補貨），或 Purchase App 對應入口

補貨報表每一列若背後有掛「訂貨點規則（Reordering Rule）」，會出現三個操作圖示：

| 圖示 | 作用 |
|------|------|
| 訂單（Order） | 立即依 Min/Max 公式（Max − 預測庫存）產生一張草稿 RFQ，需再手動確認 |
| 自動化（Automate） | 把這條規則標記為自動：之後預測庫存低於 Min，排程跑到時會自動產生草稿 RFQ，不用再手動點「訂單」 |
| 延後（Snooze） | 讓這條規則在報表上先消失一段時間，不代表問題已解決，只是先不提醒 |

> ⚠️ 沒有掛訂貨點規則的列（例如來自銷售訂單的預測需求，或已經有現成PO掛著的），不會顯示這三個圖示。判斷方法：點進該列看 Min/Max 欄位有沒有數字。

**判斷準則**：只示範單一品項、要看到「按一下就有結果」→ 用「訂單」；要示範「系統自己每天巡邏」的概念 → 用 Step 3 的「手動執行」排程，一次觸發全部符合條件的品項。

...
> 來源：demo 環境（v19-ce-jenny）開發者模式實際操作核對 + [Odoo 19 官方文件 — Access Rights](https://www.odoo.com/documentation/19.0/applications/general/users/access_rights.html)
> 背景：SOP-18 已涵蓋 Groups 表單的「存取權限」頁籤操作；本節補充「權限卡控」真正落地的技術層（Access Rights 全域清單 + Record Rules），以及先前知識庫缺口「聯絡人標籤」。

### SOP-42｜聯絡人標籤 Contact Tags

**為什麼要建置這個功能（依產業說明）：**

聯絡人標籤的核心目的是「把同一批聯絡人依不同維度快速分眾」，讓後續的篩選、報表、行銷、報價都能一鍵抓對名單。不同產業會用不同的分類邏輯：

| 產業情境 | 標籤設計範例 | 建置目的 |
|---------|------------|---------|
| 經銷商/通路業（如輪胎、零組件經銷） | 客戶/經銷商、客戶/直營、集團公司別/○○(台灣) | 同一集團底下有多個據點/子公司時，用「集團公司別」標籤把分散的聯絡人歸戶，報價、對帳、業績統計才抓得到同一集團的總量；用「經銷商/直營」區分通路層級，決定價格表與帳期政策 |
| 連鎖服務業（診所、餐飲、健身房） | VIP客戶、一般客戶、會員到期提醒 | 行銷活動/優惠訊息只發給特定分眾，不用逐筆手動篩選 |
| 製造業（供應商管理） | 原料商、代工廠、物流商 | 採購時依標籤快速篩出「哪些是原料供應商」，避免在上千筆聯絡人中人工找 |
| B2B 專案型業務 | 潛在客戶、已成交、已流失 | 業務主管快速掌握名單分布，決定資源投入優先順序 |

**顧問建置前該問客戶的問題**：「你們現在（用 Excel 或舊系統）是怎麼幫客戶/廠商分類的？」——把客戶既有的分類邏輯對應成標籤（含階層），而不是憑空設計一套新分類讓客戶重新學習。

⚠️ **前置條件**：不需開發者模式，一般使用者即可操作（需有聯絡人模組的寫入權限）

**技術名稱：** `res.partner.category`

**建立/管理標籤：**

操作路徑：聯絡人 Contacts App → 配置 Configuration → 聯絡人標籤 Contact Tags

| 欄位 | 說明 |
|------|------|
| 名稱 Name | 標籤顯示文字 |
| 類別 Category | 選填，指向另一筆標籤作為「上層分類」，用於做階層分組（僅一層 parent_id，非無限階層） |
| 顏色 Color | 從系統色板（約 11 色 + 無色）選一個，僅供視覺辨識，無其他邏輯意義 |

**階層顯示格式：** 若設定了「類別」，畫面與聯絡人清單會顯示為 `上層 / 子標籤`，例如：
- `客戶 / 經銷商`
- `集團公司別 / 駿豐(台灣)JF`

**套用到聯絡人：**

1. 開啟聯絡人表單 → 主畫面「標籤」欄位（位於 Email/電話下方，非另開頁籤）
2. 直接輸入文字：選擇現有標籤，或輸入新名稱後選「建立」即時新增
3. 可複選多個標籤（many2many，非單選）

**用途：**
- 聯絡人清單頁的篩選 Filters / 群組 Group By
- CRM／Sales／Purchase 等模組依標籤篩選客戶或廠商名單
- 行銷區隔、報表統計分類

⚠️ **重要觀念**：標籤只是分類與篩選工具，**不具備任何權限管控功能**——貼了標籤不會限制「誰看得到這筆聯絡人」。若客戶需求是「業務 A 只能看到特定區域/類型的客戶」，那是權限（Record Rules）需求，不是標籤需求，兩者常被客戶搞混，訪談時務必先釐清（見 IC-29）。

---

### SOP-43｜權限卡控技術層：存取權 Access Rights vs 記錄規則 Record Rules

**為什麼要建置這個功能（依產業說明）：**

權限卡控的目的不是「限制員工」，而是「讓每個人打開系統時看到的都是跟自己工作相關、且被允許看的資料」，降低誤操作與資料外洩風險。不同產業/組織型態，卡控的重點不同：

| 產業/組織情境 | 卡控需求 | 對應機制 |
|--------------|---------|---------|
| 多業務員通路業（如經銷商管理） | 業務只能看到自己負責的經銷商與訂單；業務主管能看全部業務的資料做管理 | Record Rules（依 salesperson/user_id 過濾）+ Access Rights（決定主管群組有全模型權限） |
| 多分公司/多品牌集團 | A 品牌員工不能誤改到 B 品牌的訂單或帳務 | Record Rules（依 company_id 過濾，多公司架構原生已內建） |
| 高敏感資料產業（診所、財務顧問、保險） | 一般員工不能看到其他客戶/患者的完整資料，只有主管或本人經手的才看得到 | Record Rules（依負責人/客戶關係過濾）+ 欄位層級權限（隱藏敏感欄位） |
| 導入初期／小型客戶 | 還不需要精細卡控，先確保「只有該用的模組看得到」即可 | 僅需 Access Rights（模組層級的無/使用者/管理員） |

**顧問建置前該問客戶的問題**：「如果業務 A 打開系統，你希望他看到所有客戶的訂單，還是只看到自己手上的？」——這句話能快速分辨客戶的需求屬於 Access Rights（能不能用）還是 Record Rules（能看多少），避免只設了模組權限就以為卡控做完了。

⚠️ **前置條件：需啟用開發者模式 Developer Mode**

**背景**：SOP-18 介紹的是「從 Groups 表單」設定單一群組的存取權限；本節介紹「從 Technical 選單」直接檢視/管理全系統的權限設定，是顧問排查權限問題時最常用的兩個畫面。

**導覽路徑：**

設定 Settings → 技術 Technical → 安全性 Security → 存取權 Access Rights ／ 記錄規則 Record Rules

| 畫面 | 技術名稱 | 網址片段（demo 環境驗證） |
|------|---------|------------------------|
| 存取權 Access Rights | `ir.model.access` | `/odoo/access-rights` |
| 記錄規則 Record Rules | `ir.rule` | `/odoo/record-rules` |

**① 存取權 Access Rights —— 控制「能不能操作這個模型」**

全系統以「模型 × 群組」為單位的 CRUD 總表（demo 環境約 900+ 筆），欄位：

| 欄位 | 說明 |
|------|------|
| 名稱 Name | 這筆存取規則的識別名稱 |
| 模型 Model | 對應的資料模型（如「付款」= account.payment） |
| 組 Group | 適用的使用者群組 |
| 讀取／寫入存取／建立存取／刪除存取 | 四個勾選框，對應 Read / Write / Create / Unlink |

這張表其實就是每個群組 Groups 表單裡「存取權限」頁籤資料的全域檢視版本——用 Technical 選單看得到「全部模型 × 全部群組」，用 Groups 表單看得到「單一群組 × 它有權限的所有模型」。

**② 記錄規則 Record Rules —— 控制「在允許操作的前提下，能看到哪些具體記錄」**
...
| 名稱 Name | 規則識別名稱 |
| 模型 Model | 對應的資料模型 |
| 群組 (無群組 = 一般) | 適用群組；留空代表全域套用（不分群組） |
| 規則定義（範圍篩選） | Domain 條件式，如 `['\|',('user_id','=',user.id),('user_id','=',False)]` |
| 已閱讀／寫入／建立／刪除 | 這條規則套用在哪些操作上 |

**實例（demo 環境驗證，模型 = 銷售訂單 sale.order）：**

| 規則名稱 | 適用群組 | 規則定義 | 效果 |
|---------|---------|---------|------|
| Personal Orders | 銷售 / 使用者 | `['\|',('user_id','=',user.id),('user_id','=',False)]` | 一般業務只看得到「自己是業務員」或「尚未指派業務員」的訂單 |
| All Orders | 銷售 / 管理員 | `[(1,'=',1)]` | 管理員看得到全部訂單，不受篩選 |

**兩層機制的判斷順序（顧問排查權限問題的標準流程）：**

1. 先問：「這個使用者能不能用『銷售訂單』這個功能？」→ 查 **Access Rights**（模型層級，總開關）
2. 再問：「他能用，但能看到哪些訂單？全部還是只有自己的？」→ 查 **Record Rules**（記錄層級，範圍篩選）
3. 兩者是疊加關係：Access Rights 沒開，Record Rules 設得再細也沒用；Access Rights 開了但沒設對應 Record Rules，使用者可能看到不該看的其他人資料

**驗證方式：** 排查使用者反映「看不到/看到不該看的資料」時，先確認其所屬 Groups（Settings → Users），再分別到 Access Rights 和 Record Rules 用「模型」篩選，逐一核對。

...
> 版本：v0.9（草稿，納入輪胎產品重建實測回饋：新增 `is_storable`／`purchase_ok` 兩個易漏產品布林欄位，回頭發現既有16筆原料主檔也漏開追蹤庫存）
...
| **產品主檔布林欄位的預設值方向常常是錯的** | `product.template` 的 `is_storable`（追蹤庫存）預設為 `False`、`purchase_ok`（可採購）預設為 `True`，若匯入範本只設計名稱/類別/屬性等「看得到」的欄位，容易漏掉這兩個「看不到但很關鍵」的布林欄位。沒開 `is_storable` 會導致後續期初庫存（`stock.quant`）完全設不了值；沒關 `purchase_ok` 會讓 BOM 產出品（如組裝完成品）錯誤出現在採購單商品選單。**教訓來源**：經銷商業態 Demo 案例輪胎產品重建時發現，且回頭查證連先前已匯入的16筆原料主檔也漏開 `is_storable`（原料收貨單走完不代表庫存真的有在追蹤水位），見 2026-07-27「產品主檔關鍵欄位」修正紀錄 |
...
| `is_storable` | Boolean | — | ⚠️ **易漏欄位**：是否追蹤庫存，**預設值為 `False`**。若此欄位不開，後續期初庫存（8.6 `stock.quant`）完全無法設定數量；BOM 產出品、原料、需要盤點的商品都必須明確設為 `True`。純服務類產品才維持 `False`（實測發現於 2026-07-27，見第5節） |
| `purchase_ok` | Boolean | — | ⚠️ **易漏欄位**：是否可被採購（出現在採購單商品選單），**預設值為 `True`**。跟 BOM 元件消耗機制（`mrp.bom.bom_line_ids` 直接指定元件產品）互相獨立、不會互相影響——外購原料/半成品保留 `True`；BOM 產出的成品（如組裝完成品）應設為 `False`，避免錯誤出現在採購選單（實測發現於 2026-07-27） |
...
| 2026-07-27 | v0.9 | 納入輪胎產品重建實測回饋（Demo案例第0週延伸任務）：① 8.3 product.template 表新增兩個易漏布林欄位——`is_storable`（追蹤庫存，預設`False`，沒開會導致期初庫存完全無法設定）、`purchase_ok`（可採購，預設`True`，BOM產出品應關閉，且此欄位與BOM元件消耗機制互相獨立不影響）；② 第5節新增「產品主檔布林欄位的預設值方向常常是錯的」一列；③ 回溯查證發現既有16筆原料主檔也漏開 `is_storable`，已一併修正，且確認P00012測試單尚未過帳、無需回溯校正庫存數字 |

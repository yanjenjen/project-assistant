...
// 成長戰略
{
 ...
 s.getRange('A3:B14').values=[['欄位','內容'],['三年定位','成為能運用 AI、資料與流程方法，協助企業診斷 ERP 營運問題、設計改善方案並推動落地的智慧流程顧問。'],['定位狀態','待本人確認；第一次季度考核校準'],['本年度主題','建立可重複的顧問工作方法與個人成長作業系統'],['年度核心成果','完成4個12週循環；形成至少4項可重用資產'],['優先服務問題','待盤點'],['主要領域／流程','待盤點'],['希望增加的工作','診斷、查證、整合、協調、落地'],['希望降低的工作','重複整理、制式產出、無目的工具學習'],['每週可投入時間','待填寫'],['外部回饋者','待指定'],['年度限制條件','工作負荷、時間、資料機密及身心狀態']];
...
['ERP與流程','跨模組流程理解','理解訂單、採購、庫存、財會等資料連動'],['ERP與流程','主資料治理','辨識編碼、責任、品質與變更控制'],['ERP與流程','內控與權責','設計授權、核准、勾稽、追溯及例外'],['ERP與流程','流程異常診斷','區分系統、資料、流程及人的原因'],['ERP與流程','產業情境理解','將標準流程調整為可落地產業作法'],
...
// 能力盤點
{
 ...
 const rows=caps.map((x,i)=>[...x,null,3,null,'E0','低','否','待本人盤點']); s.getRange(`A4:J${3+rows.length}`).values=rows;
...
// 12週計畫
{
 ...
 s.getRange('A3:B11').values=[['欄位','內容'],['循環期間','待填寫'],['核心成果','待填寫'],['核心能力','待填寫'],['支援能力1','待填寫'],['支援能力2','待填寫'],['實戰專案','待填寫'],['可重用資產','待填寫'],['成功標準','待填寫']]; header(s.getRange('A3:B3')); tableBody(s.getRange('A4:B11')); s.getRange('B4:B11').format.fill='#FFF2CC';
 s.getRange('A14:J14').values=[['週次','週成果','關鍵行動','實戰情境','所需學習','回饋來源','證據／資產','狀態','風險／阻礙','完成率']]; header(s.getRange('A14:J14'));
 const weeks=Array.from({length:12},(_,i)=>[i+1,'','','','','','','未開始','',0]); s.getRange('A15:J26').values=weeks; s.getRange('H15:H26').dataValidation={rule:{type:'list',values:['未開始','進行中','完成','延後','取消']}}; s.getRange('J15:J26').dataValidation={rule:{type:'decimal',operator:'between',formula1:0,formula2:1}}; s.getRange('J15:J26').setNumberFormat('0%'); tableBody(s.getRange('A15:J26'));
